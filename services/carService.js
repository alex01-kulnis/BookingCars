...
class CarService {
  async getFreeCars(start_d, end_d) {
    ...
    if (!this.IsValidRangeBooking(start_date, end_date)) {
      throw new Error('Нельзя забронировать машину больше чем на 30 дней');
    }

    if (!this.IsCorrectRange(start_date, end_date)) {
      throw new Error('Конечная дата бронирования не может больше начальной');
    }

    if (!this.WorkDay(start_date) || !this.WorkDay(end_date)) {
      throw new Error(
        'Начальня и конечная дата не может попадать на выходной день'
      );
    }
    ...
    const result = await db.query('select get_free_cars($1, $2) as car_id', [
      start_date,
      end_date,
    ]);
    return result;
  ...
  async getCarsReply(startMonth, endMonth) {
    let { rows } = await db.query(
      'select car_id, start_date, end_date from order_sessions'
    );
    const report = [];
  ...
  async getCarsId() {
    let { rows } = await db.query(
      'select cars.car_id from cars left join order_sessions on cars.car_id = order_sessions.car_id where order_sessions.car_id is null'
    );
    const ids = [];
  ...
  getWorkload(startMonth, endMonth, startOrder, endOrder) {
    ...
    if (startOrder >= startMonth && endOrder <= endMonth)
      return (
        (this.getDaysBetweenDates(startOrder, endOrder) / daysInMonth) * 100
      );

    if (
      startOrder >= startMonth &&
      startOrder <= endMonth &&
      endOrder > endMonth
    )
      return (
        (this.getDaysBetweenDates(endMonth, startMonth) / daysInMonth) * 100
      );
    ...
    if (
      ...
      return (
        (this.getDaysBetweenDates(endOrder, startMonth) / daysInMonth) * 100
      );

    if (startOrder < startMonth && endOrder > endMonth) return 1;
